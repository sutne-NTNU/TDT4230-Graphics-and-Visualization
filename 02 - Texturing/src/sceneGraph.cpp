...
static int lightIdCounter = 0;
/**
 * @brief Create a Scene Node With light attributes
 *
 * @param type type of light (POINT_LIGHT or SPOT_LIGHT)
 */
SceneNode *createLightNode(SceneNodeType type)
{
    SceneNode *node  = createSceneNode();
    node->type       = type;
    node->lightID    = lightIdCounter++;
    node->lightColor = glm::vec3(1, 1, 1);
    return node;
}
